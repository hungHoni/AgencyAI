    ...
    <section id="how-it-works" ref={ref} className="max-w-site mx-auto px-12 py-20 max-sm:px-5 max-sm:py-14 relative z-10">
      ...
      <div className="grid grid-cols-[1fr_1.1fr] gap-24 items-center max-lg:grid-cols-1 max-lg:gap-16">
        ...
        <div style={{ opacity: visible ? 1 : 0, transform: visible ? "translateX(0)" : "translateX(-24px)", transition: "opacity 0.7s cubic-bezier(0.16, 1, 0.3, 1), transform 0.7s cubic-bezier(0.16, 1, 0.3, 1)" }}>
          <div className="text-xs font-semibold text-blue-500 dark:text-blue-400 tracking-[1.2px] uppercase mb-4">{t("howItWorks.sectionTag")}</div>
          ...
          <p className="text-lg text-[var(--text-secondary)] leading-[1.7] mb-12 max-w-[480px]">{t("howItWorks.subtext")}</p>

          <div className="flex flex-col relative max-w-[500px]">
            {/* Background absolute line */}
            <div className="absolute left-[23px] top-4 bottom-12 w-[2px] bg-[var(--border)]" />
            ...
            {data.howItWorks.steps.map((step, i) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`relative z-20 w-12 h-12 rounded-full flex items-center justify-center text-lg font-bold tracking-[-0.5px] shrink-0 transition-all duration-500 ${isActive
                      ? "bg-blue-500 text-white shadow-[0_0_24px_rgba(96,165,250,0.5)] scale-110"
                      : isPast
                        ? "bg-blue-500 text-white"
                        : "bg-[var(--bg-card)] border-2 border-[var(--border-strong)] text-[var(--text-muted)] group-hover:border-[var(--text-muted)]"
                      }`}
                  >
                    {i + 1}
                  </div>
                  ...
                  <div className={`pt-2 transition-all duration-500 ${isActive ? "opacity-100 translate-x-2" : "opacity-40 hover:opacity-70"}`}>
                    <h4 className={`text-xl font-bold tracking-[-0.3px] mb-2 ${isActive ? "text-[var(--text-primary)]" : "text-[var(--text-secondary)]"}`}>{step.title}</h4>
                    <p className="text-base text-[var(--text-secondary)] leading-[1.6] max-w-[340px]">{step.description}</p>
                  </div>
                </div>
              );
            })}
        ...
        >
          ...
          <div className={`absolute inset-0 flex items-center justify-center transition-all duration-700 ease-in-out z-10 ${activeStep === 0 ? "opacity-100 scale-100" : "opacity-0 scale-95 pointer-events-none"}`}>
            <div className="w-[300px] h-[340px] bg-[var(--bg-card)] rounded-3xl shadow-[var(--shadow-elevated)] border border-[var(--border)] flex flex-col py-7 px-7 relative overflow-hidden">
              <div className="absolute top-0 right-0 w-32 h-32 bg-blue-400/10 rounded-full blur-2xl" />

              <div className="flex justify-between items-center mb-8 relative z-10">
                <div className="w-24 h-4 bg-[var(--text-muted)]/20 rounded-full" />
              ...
              <div className="grid grid-cols-4 gap-3 mb-8 relative z-10">
                {[...Array(12)].map((_, j) => (
                  <div key={j} className={`h-12 rounded-[10px] transition-all duration-500 ${j === 6 ? 'bg-blue-500 shadow-lg shadow-blue-500/25 scale-110' : 'bg-[var(--bg)] dark:bg-white/[0.04]'}`} />
                ))}
              </div>

              <div className="mt-auto h-12 w-full bg-zinc-900 dark:bg-zinc-800 rounded-[12px] flex items-center justify-center relative overflow-hidden shrink-0">
          ...
          <div className={`absolute inset-0 flex items-center justify-center transition-all duration-700 ease-in-out z-10 ${activeStep === 2 ? "opacity-100 scale-100" : "opacity-0 scale-95 pointer-events-none"}`}>
            <div className="w-[380px] bg-[var(--bg-card)] rounded-2xl shadow-[var(--shadow-elevated)] border border-[var(--border)] overflow-hidden flex flex-col">
              <div className="h-10 bg-[var(--bg)] border-b border-[var(--border)] flex items-center px-4 gap-2">
              ...
              <div className="p-8 flex flex-col justify-center items-center h-[260px] relative overflow-hidden bg-gradient-to-b from-blue-50/50 dark:from-blue-500/5 to-transparent">
                ...
                <div className="w-3/4 h-5 bg-zinc-800 dark:bg-zinc-200 rounded-full mb-3" />
                <div className="w-1/2 h-4 bg-[var(--text-muted)]/20 rounded-full" />
              </div>
            </div>
          </div>
