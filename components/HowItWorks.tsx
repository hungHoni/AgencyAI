    ...
    <section id="how-it-works" ref={ref} className="max-w-site mx-auto px-12 py-20 max-sm:px-5 max-sm:py-14 relative z-10">
      ...
      <div className="grid grid-cols-[1fr_1.1fr] gap-24 items-center max-lg:grid-cols-1 max-lg:gap-16">
        ...
        <div style={{ opacity: visible ? 1 : 0, transform: visible ? "translateX(0)" : "translateX(-24px)", transition: "opacity 0.7s cubic-bezier(0.16, 1, 0.3, 1), transform 0.7s cubic-bezier(0.16, 1, 0.3, 1)" }}>
          ...
          <p className="text-lg text-zinc-600 leading-[1.7] mb-12 max-w-[480px]">{t("howItWorks.subtext")}</p>
              ...
              return (
                ...
                >
                  ...
                  <div className={`pt-2 transition-all duration-500 ${isActive ? "opacity-100 translate-x-2" : "opacity-40 hover:opacity-70"}`}>
                    <h4 className={`text-xl font-bold tracking-[-0.3px] mb-2 ${isActive ? "text-zinc-900" : "text-zinc-600"}`}>{step.title}</h4>
                    <p className="text-base text-zinc-600 leading-[1.6] max-w-[340px]">{step.description}</p>
                  </div>
                </div>
              );
            })}
