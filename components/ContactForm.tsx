...
export default function ContactForm() {
  ...
  const trustIcons = [
    <svg key="phone" viewBox="0 0 24 24" className="w-[18px] h-[18px] stroke-emerald-500 fill-none" strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z" /></svg>,
    <svg key="clock" viewBox="0 0 24 24" className="w-[18px] h-[18px] stroke-emerald-500 fill-none" strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg>,
    <svg key="shield" viewBox="0 0 24 24" className="w-[18px] h-[18px] stroke-emerald-500 fill-none" strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" /></svg>,
  ];
  ...
  return (
    <section id="contact" ref={sectionRef} className="max-w-site mx-auto px-12 py-24 grid grid-cols-2 gap-20 items-start max-lg:grid-cols-1 max-lg:gap-12 max-sm:px-5 max-sm:py-16">
      <div style={{ opacity: visible ? 1 : 0, transform: visible ? "translateY(0)" : "translateY(24px)", transition: "opacity 0.7s cubic-bezier(0.16, 1, 0.3, 1), transform 0.7s cubic-bezier(0.16, 1, 0.3, 1)" }}>
        <div className="text-xs font-semibold text-emerald-600 tracking-[1.2px] uppercase mb-3">{t("contact.sectionTag")}</div>
        ...
        <div className="mt-10 flex flex-col gap-5">
          {data.contact.trustSignals.map((signal, i) => (
            <div key={i} className="flex items-start gap-3.5">
              <div className="w-10 h-10 bg-[rgba(16,185,129,0.06)] rounded-btn flex items-center justify-center shrink-0">{trustIcons[i]}</div>
              <div>
          ...
          <form onSubmit={handleSubmit}>
            <div className="grid grid-cols-2 gap-3.5 max-sm:grid-cols-1">
              <div className="mb-4">
                ...
                <input type="text" placeholder={t("contact.formPlaceholders.name")} value={name} onChange={(e) => setName(e.target.value)} className="w-full bg-[#fafaf9] border border-black/10 rounded-btn py-[13px] px-3.5 text-sm outline-none transition-all duration-400 ease-smooth focus:border-emerald-500 focus:shadow-[0_0_0_3px_rgba(16,185,129,0.06)] placeholder:text-zinc-400" />
                {errors.name && <p className="text-red-500 text-xs mt-1">{errors.name}</p>}
              </div>
              <div className="mb-4">
                <label className="block text-[13px] font-semibold mb-1.5 tracking-[-0.1px]">{t("contact.formLabels.business")}</label>
                <input type="text" placeholder={t("contact.formPlaceholders.business")} value={business} onChange={(e) => setBusiness(e.target.value)} className="w-full bg-[#fafaf9] border border-black/10 rounded-btn py-[13px] px-3.5 text-sm outline-none transition-all duration-400 ease-smooth focus:border-emerald-500 focus:shadow-[0_0_0_3px_rgba(16,185,129,0.06)] placeholder:text-zinc-400" />
            ...
            <div className="mb-4">
              ...
              <input type="email" placeholder={t("contact.formPlaceholders.email")} value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-[#fafaf9] border border-black/10 rounded-btn py-[13px] px-3.5 text-sm outline-none transition-all duration-400 ease-smooth focus:border-emerald-500 focus:shadow-[0_0_0_3px_rgba(16,185,129,0.06)] placeholder:text-zinc-400" />
              {errors.email && <p className="text-red-500 text-xs mt-1">{errors.email}</p>}
            </div>
            <div className="mb-4">
              <label className="block text-[13px] font-semibold mb-1.5 tracking-[-0.1px]">{t("contact.formLabels.message")}</label>
              <textarea placeholder={t("contact.formPlaceholders.message")} value={message} onChange={(e) => setMessage(e.target.value)} className="w-full bg-[#fafaf9] border border-black/10 rounded-btn py-[13px] px-3.5 text-sm outline-none transition-all duration-400 ease-smooth focus:border-emerald-500 focus:shadow-[0_0_0_3px_rgba(16,185,129,0.06)] placeholder:text-zinc-400 h-[100px] resize-y" />
            </div>
